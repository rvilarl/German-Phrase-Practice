...
const AnalyticsCard: React.FC<{ label: string; primary: string; secondary?: string }> = ({ label, primary, secondary }) => (
...
const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const decimalFormatter = useMemo(() => new Intl.NumberFormat(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 }), []);
      ...
      >
        ...
        <div className="relative h-[480px] overflow-hidden">
            ...
            <div className={`absolute inset-0 p-6 space-y-4 transition-transform duration-300 ease-in-out ${view === 'main' ? 'translate-x-0' : '-translate-x-full'}`}>
                <button onClick={() => setView('analytics')} className="w-full flex items-center justify-between p-4 bg-slate-700/50 hover:bg-slate-700 rounded-lg transition-colors">
                    <span className="text-slate-200">{t('settings.views.analytics')}</span>
            ...
            <div className={`absolute inset-0 p-6 space-y-6 transition-transform duration-300 ease-in-out hide-scrollbar overflow-y-auto ${view === 'analytics' ? 'translate-x-0' : 'translate-x-full'}`}>
                  ...
                  <div className="bg-slate-900/40 border border-slate-700/60 rounded-xl p-4 shadow-inner">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-100 font-semibold">{formatPercentValue(practiceAnalyticsSummary.accuracy.overall)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-300">{t('settings.analytics.srs.last7Days')}</span>
                        <span className="text-slate-100 font-semibold">{formatPercentValue(practiceAnalyticsSummary.accuracy.last7Days)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-300">{t('settings.analytics.srs.last30Days')}</span>
                        <span className="text-slate-100 font-semibold">{formatPercentValue(practiceAnalyticsSummary.accuracy.last30Days)}</span>
                      </div>
                    </div>
                  </div>
                      ...
                      {practiceAnalyticsSummary.levels.map(level => {
                        ...
                        return (
                          <div key={level.level} className="flex items-center gap-3">
                            <span className="text-xs text-slate-400 w-12">{t('settings.analytics.srs.level', { level: level.level })}</span>
                          ...
                          <div key={cat.id} className="bg-slate-800/60 border border-slate-700/60 rounded-lg p-3">
                            ...
                            <div className="grid grid-cols-3 gap-2 text-xs">
                              ...
                              <div>
                                ...
                                <p className="text-slate-100 font-semibold">{decimalFormatter.format(cat.avgMasteryLevel)}</p>
                              </div>
                              <div>
                        ...
                        {recentReviews.map(day => {
                          ...
                          return (
                            <div key={day.date} className="flex-1 flex flex-col items-center">
                              <div className="w-full bg-slate-700/60 rounded-t relative" style={{ height: `${Math.max(heightPercent, 5)}%` }}>
                                <div
                    ...
                    {practiceAnalyticsSummary.leeches.length > 0 ? (
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                        {practiceAnalyticsSummary.leeches.slice(0, 10).map(leech => (
                          <div key={leech.phraseId} className="bg-slate-800/60 border border-red-900/40 rounded-lg p-2 flex justify-between items-center">
