    ...
    return text.split(' ').map((word, i, arr) => (
      <span
        ...
        onContextMenu={(e) => {
          ...
          e.stopPropagation();
          const cleaned = word.replace(/[.,!?()""":;]/g, '');
          if (cleaned && onOpenContextMenu) onOpenContextMenu({ sentence: { german: text, native: translation }, word: cleaned });
        }}
...
export const PracticeChatModal_v2: React.FC<Props> = ({
  ...
  const normalizeAssistantTranslation = useCallback(async (message: PracticeChatMessage) => {
    ...
    try {
      const translationResult = await onTranslateGermanToNative(primaryContent.text);
